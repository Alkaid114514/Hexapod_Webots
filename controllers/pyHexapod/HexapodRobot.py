...
class Hexapod(Robot):
    def __init__(self):
        ...
        self.BRleg = LegR(
            self.getDevice("M_BR_COXA"),
            self.getDevice("M_BR_FEMUR"),
            self.getDevice("M_BR_TIBIA"),
            self.ctr2BRroot,
            self.ctr2BRrootTheta,
        )
        self.MRleg = LegR(
            self.getDevice("M_MR_COXA"),
            self.getDevice("M_MR_FEMUR"),
            self.getDevice("M_MR_TIBIA"),
            self.ctr2MRroot,
            self.ctr2MRrootTheta,
        )
        self.FRleg = LegR(
            self.getDevice("M_FR_COXA"),
            self.getDevice("M_FR_FEMUR"),
            self.getDevice("M_FR_TIBIA"),
            self.ctr2FRroot,
            self.ctr2FRrootTheta,
        )
        self.BLleg = LegL(
            self.getDevice("M_BL_COXA"),
            self.getDevice("M_BL_FEMUR"),
            self.getDevice("M_BL_TIBIA"),
            self.ctr2BLroot,
            self.ctr2BLrootTheta,
        )
        self.MLleg = LegL(
            self.getDevice("M_ML_COXA"),
            self.getDevice("M_ML_FEMUR"),
            self.getDevice("M_ML_TIBIA"),
            self.ctr2MLroot,
            self.ctr2MLrootTheta,
        )
        self.FLleg = LegL(
            self.getDevice("M_FL_COXA"),
            self.getDevice("M_FL_FEMUR"),
            self.getDevice("M_FL_TIBIA"),
            self.ctr2FLroot,
            self.ctr2FLrootTheta,
        )
        ...
        leg_coords = self.body2legCoord(
            self.FLleg.initStandBodyTarget,
            self.FLleg.ctr2root,
            self.FLleg.ctr2rootTheta,
        )
    ...
    def collect_sensor_data(self):
        ...
        sensor_data = {}
        if self.lidar:
            list = self.lidar.get_point_cloud()[::50]
            list1 = self.lidar.get_range_image()[::50]
            points = []
            rangeImage = []
            for i, v in enumerate(list):
                if not math.isinf(v.x) and list1[i] < 4:
                    points.append({"x": v.x, "y": v.y, "z": v.z})
                    rangeImage.append(list1[i])
                    if len(points) > 600:
                        break
            sensor_data = {"points": points, "rangeImage": rangeImage}
        return sensor_data
    ...
    def prepareNextCycle(self, moveStatus):
        if self.isGaitCycleStart() or self.gaitStatus == GaitStatus.Stop:
            ...
            self.totalFrame = (
                24 if isVzero else int(math.ceil(32 * (0.01 / vel_magnitude_sq)))
            )

            mr_target = self.MRleg.currentStandBodyTarget
            if isVzero:
                self.stepLen = self.lockedOmega * math.sqrt(
                    mr_target[0] ** 2 + mr_target[1] ** 2
                )
            else:
                self.stepLen = (
                    self.lockedOmega / abs(self.lockedOmega)
                ) * np.linalg.norm(self.lockedVelocity)

            self.stepLen *= (self.timeStep / 1000.0) * self.totalFrame

            self.lockedR = np.cross(self.lockedVelocity, w) / (self.lockedOmega**2)
            rlen = (
                math.sqrt(mr_target[0] ** 2 + mr_target[1] ** 2)
                if isVzero
                else np.linalg.norm(self.lockedR)
            )
    ...
    def getStandNextBodyTarget(
        self, r0, currentStandBodyTarget, baseRatio=1.0, ratio=0.0
    ):
        pc = r0 + currentStandBodyTarget
        theta = (
            self.stepTheta * ratio
            + self.stepTheta * baseRatio * self.frame / self.totalFrame
            - self.stepTheta / 2.0
        )
        rotation_matrix = np.array(
            [
    ...
    def setBodyTargets(
        self, BRtarget, MRtarget, FRtarget, BLtarget, MLtarget, FLtarget
    ):
        self.BRleg.setLegTarget(
            self.body2legCoord(BRtarget, self.BRleg.ctr2root, self.BRleg.ctr2rootTheta)
        )
        self.MRleg.setLegTarget(
            self.body2legCoord(MRtarget, self.MRleg.ctr2root, self.MRleg.ctr2rootTheta)
        )
        self.FRleg.setLegTarget(
            self.body2legCoord(FRtarget, self.FRleg.ctr2root, self.FRleg.ctr2rootTheta)
        )
        self.BLleg.setLegTarget(
            self.body2legCoord(BLtarget, self.BLleg.ctr2root, self.BLleg.ctr2rootTheta)
        )
        self.MLleg.setLegTarget(
            self.body2legCoord(MLtarget, self.MLleg.ctr2root, self.MLleg.ctr2rootTheta)
        )
        self.FLleg.setLegTarget(
            self.body2legCoord(FLtarget, self.FLleg.ctr2root, self.FLleg.ctr2rootTheta)
        )
    ...
    def moveRipple(self):
        ...
        if self.gaitGroupIndex == 0:
            self.FLleg.setBodyTarget(
                self.getSwagNextBodyTarget(
                    self.lockedR, self.FLleg.currentStandBodyTarget
                )
            )
            self.MRleg.setBodyTarget(
                self.getSwagNextBodyTarget(
                    self.lockedR, self.MRleg.currentStandBodyTarget
                )
            )
            self.FRleg.setBodyTarget(
                self.getStandNextBodyTarget(
                    self.lockedR,
                    self.FRleg.currentStandBodyTarget,
                    RIPPLE_RATIO,
                    1.0 * RIPPLE_RATIO,
                )
            )
            self.MLleg.setBodyTarget(
                self.getStandNextBodyTarget(
                    self.lockedR,
                    self.MLleg.currentStandBodyTarget,
                    RIPPLE_RATIO,
                    2.0 * RIPPLE_RATIO,
                )
            )
            self.BRleg.setBodyTarget(
                self.getStandNextBodyTarget(
                    self.lockedR,
                    self.BRleg.currentStandBodyTarget,
                    RIPPLE_RATIO,
                    2.0 * RIPPLE_RATIO,
                )
            )
            self.BLleg.setBodyTarget(
                self.getStandNextBodyTarget(
                    self.lockedR,
                    self.BLleg.currentStandBodyTarget,
                    RIPPLE_RATIO,
                    1.0 * RIPPLE_RATIO,
                )
            )
            ...
            self.MLleg.setBodyTarget(
                self.getSwagNextBodyTarget(
                    self.lockedR, self.MLleg.currentStandBodyTarget
                )
            )
            self.BRleg.setBodyTarget(
                self.getSwagNextBodyTarget(
                    self.lockedR, self.BRleg.currentStandBodyTarget
                )
            )
            self.FLleg.setBodyTarget(
                self.getStandNextBodyTarget(
                    self.lockedR,
                    self.FLleg.currentStandBodyTarget,
                    RIPPLE_RATIO,
                    1.0 * RIPPLE_RATIO,
                )
            )
            ...
            self.MRleg.setBodyTarget(
                self.getStandNextBodyTarget(
                    self.lockedR,
                    self.MRleg.currentStandBodyTarget,
                    RIPPLE_RATIO,
                    1.0 * RIPPLE_RATIO,
                )
            )
            ...
            self.BLleg.setBodyTarget(
                self.getSwagNextBodyTarget(
                    self.lockedR, self.BLleg.currentStandBodyTarget
                )
            )
            self.FRleg.setBodyTarget(
                self.getSwagNextBodyTarget(
                    self.lockedR, self.FRleg.currentStandBodyTarget
                )
            )
            self.MLleg.setBodyTarget(
                self.getStandNextBodyTarget(
